...
## Ajustes de configuración para inicio rápido

<Steps>

1. **Descargar la aplicación móvil**

    Descarga la aplicación móvil Meshtastic desde [Google Play](https://play.google.com/store/apps/details?id=com.geeksville.mesh) o [App Store](https://apps.apple.com/us/app/meshtastic/id1532238235).

2. **Conectar por Bluetooth**

    Conecta tu dispositivo Meshtastic vía Bluetooth, busca el dispositivo en la lista y empareja con el código de aparece en la pantalla del nodo o con el código preconfigurado que te indique el fabricante en el manual de usuario en caso de que el nodo no tenga pantalla.

3. **Configurar usuario**

    Configura un nombre largo y un nombre corto para tu nodo. Puedes usar el nombre que desees, pero te recomendamos usar un nombre que te permita identificar fácilmente el nodo.

    {/* También puedes usar la herramienta de [generador de nombres de nodos](https://meshcolombia.co/herramientas/generador-nombres-nodos/) para obtener ideas. */}

4. **Configurar canales**

    Configura el canal primario (**LongFast**) de la siguiente manera:
        - Llave (**Key**): `AQ==`
        - Enlace de subida (**Enable Uplink**): `Habilitado`
        - Enlace de bajada (**Enable Downlink**): `Habilitado`
        - Reporte de ubicación (**Map Reporting**): `Habilitado`

5. **Configurar el módulo LoRa**

    Configura el módulo LoRa de la siguiente manera:
        - Región: `ANZ (Australia y Nueva Zelanda)`
        - Slot: `20`
        - Habilitar la opción: `OK to MQTT`

6. **Configuración de MQTT**

    Configura MQTT de la siguiente manera:
        - Servidor (**Server**): `mqtt.meshcolombia.co`
        - Usuario (**User**): `meshcousers`
        - Contraseña (**Password**): `meshcousers`
        - Root Topic: `msh/CO`
        - Encriptación (**Enable Encryption**): `Habilitado`
        - Salida JSON (**Enable JSON Output**): `Deshabilitado`
        - TLS: `Deshabilitado`
        - Si estás usando WiFi, deshabilita la opción `Proxy to Client Enabled`, Si estás usando un teléfono móvil, habilita la opción `Proxy to Client Enabled`.

    7. **Guardar y reiniciar**

        Guarda (envía) los cambios y reinicia el dispositivo para que los nuevos ajustes tengan efecto.

</Steps>

## Ajustes de configuración detallados
